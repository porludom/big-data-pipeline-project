...
with psql.connect(connString) as conn:
    ...
    with open(os.path.join("sql", "create_db.sql"), encoding="utf-8") as file:
        content = file.read()
        cur.execute(content)
    conn.commit()
