...
with psql.connect(connString) as conn:
    ...
    with open(os.path.join("../project/big-data-pipeline-project/sql", "create_db.sql"), encoding="utf-8") as file:
        content = file.read()
        cur.execute(content)
    conn.commit()

    # Read the commands from the file and execute them.
    with open(os.path.join("../project/big-data-pipeline-project/sql", "import_data.sql"),
              encoding="utf-8") as file:
        commands = file.readlines()
        with open(os.path.join("../project/big-data-pipeline-project/data", "locations.csv"), "r",
                  encoding="utf-8") as locations:
            cur.copy_expert(commands[0], locations)
        with open(os.path.join("../project/big-data-pipeline-project/data", "weather_conditions.csv"), "r",
                  encoding="utf-8") as weather_conditions:
            cur.copy_expert(commands[1], weather_conditions)
        with open(os.path.join("../project/big-data-pipeline-project/data", "astronomical_data.csv"), "r",
                  encoding="utf-8") as astronomical_data:
            cur.copy_expert(commands[2], astronomical_data)
        with open(os.path.join("../project/big-data-pipeline-project/data", "air_quality.csv"), "r",
